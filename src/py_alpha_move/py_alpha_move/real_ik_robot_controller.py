...
class RobotController(Node):
    ...
    def __init__(self):
        ...
        self.commands_pub = self.create_publisher(Float64MultiArray, '/forward_position_controller/commands', 10)  # New publisher
    ...
    def ik_callback(self, msg):
        ...
        if len(joint_poses) >= 5:
            ...
            self.get_logger().info(f"Published Joint Positions to /alpha_move_joint: {joint_poses}")
            
            commands_msg = Float64MultiArray()
            commands_msg.data = joint_poses  
            self.commands_pub.publish(commands_msg)
            self.get_logger().info(f"Published Joint Positions to /forward_position_controller/commands: {joint_poses}")
