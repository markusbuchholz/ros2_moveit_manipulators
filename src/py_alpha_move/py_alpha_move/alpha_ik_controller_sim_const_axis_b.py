...
class IKSolverNode(Node):
    def __init__(self):
        ...
        # Initial guess (axis_a is fixed at 0.0, not included here)
    ...
    def target_position_callback(self, msg):
        ...
        if joint_angles is not None:
            # Reverse the joint angles to match the joint_names order (axis_b, axis_c, axis_d, axis_e)
            joint_angles_reversed = joint_angles[::-1]

            # **Fix axis_b to -1.0 radians**
            # joint_angles_reversed[0] corresponds to axis_b after reversal
            joint_angles_reversed[0] = -1.0

            # Insert axis_a fixed at 0.0 radians
            ...
            # Construct the full joint angles including the fixed axis_b
            full_joint_angles = joint_angles_published[1:].copy()  # Exclude axis_a which is fixed at 0.0
            full_joint_angles[0] = -1.0  # axis_b is fixed

            computed_position = forward_kinematics(full_joint_angles)
